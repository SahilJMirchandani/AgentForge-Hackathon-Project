...
async function handle(req, res) {
  ...
  try {
    ...
    if (parts[1] === 'workflows' && req.method === 'DELETE') {
      ...
      delete db.workflows[workflowId]; notify(user, 'Workflow removed', 'The workflow was successfully deleted.'); saveDb(db).catch((error) => console.warn(`Workflow delete persistence warning: ${error.message}`)); return json(res, 200, { ok: true, notifications: db.notifications[user.email] })
    }
    ...
    if (parts[1] === 'workflows' && parts[3] === 'deploy' && req.method === 'POST') {
      ...
      workflow.isDeployed = true; workflow.isActive = true; workflow.webhookToken ||= randomBytes(32).toString('hex'); if (/\bgmail\b|\binbox\b|new email.*arriv|monitor.*email|check.*email|read.*email/i.test(`${workflow.prompt || ''} ${(workflow.nodes || []).map((node) => `${node.data?.title || ''} ${node.data?.instructions || ''}`).join(' ')}`) && !workflow.gmailLastSeenAt) workflow.gmailLastSeenAt = Date.now(); notify(user, 'Agent deployed', `${workflow.name} is live and ready to receive events.`); saveDb(db).catch((error) => console.warn(`Deployment persistence warning: ${error.message}`)); const publicApiOrigin = appConfig.publicApiOrigin || `${req.headers['x-forwarded-proto'] || 'http'}://${req.headers.host}`; const webhookUrl = `${publicApiOrigin.replace(/\/$/, '')}/api/hooks/${workflow.webhookToken}`; return json(res, 200, { workflow: publicWorkflow(workflow), notifications: db.notifications[user.email], webhookUrl }, allowedOrigin)
    }
