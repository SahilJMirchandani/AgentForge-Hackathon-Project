    ...
    try {
      ...
      for (const workflow of Object.values(db.workflows || {})) {
        ...
        if (!run.skipNotifications) {
          ...
          notify(owner, run.status === 'Completed' ? (schedule.gmail ? 'New email processed' : 'Scheduled agent completed') : (schedule.gmail ? 'Email agent failed' : 'Scheduled agent failed'), run.status === 'Completed' ? (schedule.gmail ? `${workflow.name || 'Email agent'} detected and processed a new email. Summary:\n${summaryText || 'Summary generated successfully.'}` : `${workflow.name} completed its scheduled run.`) : run.error)
        }
      }
      await saveDb(db)
    } finally {
